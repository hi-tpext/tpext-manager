...
class CreatorLogic
{
    ...
    {
        $appPath = App::getRootPath() . str_replace(['\\', '/'], DIRECTORY_SEPARATOR, $modelNamespace == 'app\\admin\\model' ? 'app/admin/model' : 'app/common/model');
        ...
        while (false !== ($file = readdir($dir))) {
            ...
            if (($file != '.') && ($file != '..')) {
                ...
                if (is_dir($sonDir)) {
                    ...
                    if (preg_match('/.+?\\\app\\\(admin|common)(\\\model\\\.+?)\.php$/i', $sonDir, $mtches)) {
    ...
    protected function parseModel($thisClass, $relationClass)
    {
        if (false === strpos($relationClass, '\\')) {
            ...
            array_pop($path);
            array_push($path, Str::studly($relationClass));
